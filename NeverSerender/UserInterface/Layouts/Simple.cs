...
namespace NeverSerender.UserInterface.Layouts
{
    internal class Simple : Layout
    {
        ...
        public override Vector2 PanelSize => new Vector2(0.5f, 0.7f);
        ...
        public override List<MyGuiControlBase> RecreateControls()
        {
            parent = new MyGuiControlParent
            {
                ...
                Size = new Vector2(PanelSize.X - 0.01f, PanelSize.Y - 0.09f)
            };
            ...
            foreach (var control in GetControls().SelectMany(row => row))
        ...
        public override void LayoutControls()
        {
            ...
            var totalHeight = ElementPadding +
            ...
            foreach (var row in controls)
            {
                ...
                foreach (var control in row)
                {
                    ...
                    {
                        guiControl.Size = new Vector2(Math.Max(control.MinWidth, unitWidth * control.FillFactor.Value),
                            sizeY);
                    }
                    else
                    {
