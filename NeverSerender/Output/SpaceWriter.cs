...
namespace NeverSerender.Output
{
    public class SpaceWriter
    {
        ...
        public void End() => writer.Event(Event.End, w =>
        {
            w.Write((uint)0);
            w.PropertyEnd();
        });
        ...
        public void Advance(float delta) => writer.Event(Event.Advance, w =>
        {
        ...
        public void Light(uint id, MatrixD matrix, Vector3 color, Vector2? cone) => writer.Event(Event.Light, w =>
        {
        ...
        public void RemoveLight(uint id) => writer.Event(Event.Light, w =>
        {
        ...
        public void RemoveEntity(uint id) => writer.Event(Event.Entity, w =>
        {
        ...
        public void Entity(uint id, EntitySnapshot entity, uint? model) => writer.Event(Event.Entity, w =>
        {
            log.WriteLine($"Entity id={id} parent={entity.Parent} name={entity.Name} model={entity.Model} color={entity.Color} preview={entity.IsPreview}");
            w.Write(id);
        ...
        public void Grid(uint id, float scale, IList<BlockProperties> blocks) => writer.Event(Event.Grid, w =>
        {
        ...
        public void Material(uint id, MaterialProperties props) => writer.Event(Event.Material,
            w =>
            {
                w.Write(id);
                w.PropertyEnd();
        ...
        public void Texture(uint id, TextureType type, string name, string path,
            Action<Stream> writeTexture) => writer.Event(Event.Texture, w =>
        {
            w.Write(id);
